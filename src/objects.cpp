...
bool Camera::sees(Point3D a,Point3D b,Point3D c,Point3D d){
    ...
    this->leftLimit.x = -(this->position.x + limitLength * cos(this->latitude + this->fov*M_PI/360.));
    this->leftLimit.y = -(this->position.y + limitLength * sin(this->latitude + this->fov*M_PI/360.));
    this->leftLimit.z = -(this->position.z);

    this->rightLimit.x = this->position.x + limitLength * cos(this->latitude - this->fov*M_PI/360.);
    this->rightLimit.y = this->position.y + limitLength * sin(this->latitude - this->fov*M_PI/360.);
